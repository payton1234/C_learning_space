...
struct token_queue infix_to_postfix(struct token_queue * pqueue_infix) {
	...
	struct token_queue output_queue;
	p_expr_token current = pqueue_infix->front, stack_top;
	output_queue.front = NULL;
	output_queue.back = NULL;
	for (current = dequeue(pqueue_infix); current != NULL; current = dequeue(pqueue_infix)) {
		if (current->type == OPERAND) {
			enqueue(&output_queue, current);
		}
		else {
			if (stack_top == NULL || op_precedences[current->value.op_code] > op_precedences[stack_top->value.op_code]) {
				push(&stack_top, current);
			}
			else if (op_precedences[current->value.op_code] <= op_precedences[stack_top->value.op_code]) {
				while(stack_top != NULL) {
					enqueue(&output_queue, (pop(&stack_top)));
				}
				push(&stack_top, current);					
			}
		}
...
double evaluate_postfix(struct token_queue * pqueue_postfix) {
	...
	union token_value value;
	//union token_value value;
	p_expr_token stack_top =NULL, current;
	for (current = dequeue(pqueue_postfix); current != NULL; current = dequeue(pqueue_postfix)) {
		if (current->type == OPERAND) {
			push(&stack_top, current);
		}
		else {
			for (i = 0; i < op_operands[current->value.op_code]; i++) {
				operands[i] = pop(&stack_top)->value.operand;
			}
			switch (current->value.op_code) {
				case ADD:
